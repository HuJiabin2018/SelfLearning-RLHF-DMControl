...
from reward_model_self import RewardModel, gen_net, KCenterGreedy
...
class RewardModelSemiDataAug(RewardModel):
    def __init__(self, ds, da, 
        ...
        self.learning_fail_inputs = []
        self.learning_fail_targets = []
        self.temp_input = []
        self.temp_target = []

        self.buffer_seg1_self = np.empty((self.capacity, self.size_segment, self.ds + self.da), dtype=np.float32)
        self.buffer_seg2_self = np.empty((self.capacity, self.size_segment, self.ds + self.da), dtype=np.float32)
    ...
    def kcenter_sampling(self, isSelf=False):
        ...
        u_sa_t_1, u_sa_t_2, u_r_t_1, u_r_t_2 = self.get_queries(mb_size=self.mb_size * self.inv_label_ratio, isSelfLearning=isSelf)
